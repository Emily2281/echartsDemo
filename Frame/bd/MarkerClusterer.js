/**
 * source: //api.map.baidu.com/library/MarkerClusterer/1.2/src/MarkerClusterer_min.js
 * change by yls on 2020-02-06
 */
...
(function() {
    ...
    MarkerClusterer.prototype._addToClosestCluster = function(marker) {
        ...
        if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)) {
            ...
            // change by yls
            // 触发自定义事件，触发第一个点聚合中的marker事件
            const _m = marker;
            cluster._clusterMarker.onmouseover = function(e) {//在每个聚合点对象的视图对象上绑定事件
                //这样就可以为每一个聚合点绑定点击事件了
                var myEvent = new CustomEvent('clusterMouseOver', {
                    detail: {cluster: cluster},
                });
                // 随后在对应的元素上触发该事件
                if(_m.dispatchEvent) {
                    _m.dispatchEvent(myEvent);
                } else {
                    _m.fireEvent(myEvent);
                }
            }
